...
[![PyPI](https://img.shields.io/pypi/v/cognis-mftparse.svg)](https://pypi.org/project/cognis-mftparse/)
[![CI](https://github.com/cognis-digital/mftparse/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/mftparse/actions)
...
MFTPARSE exists for one job — analyze an ntfs $mft csv for timestomping and suspicious file activity — and does it without a SaaS bill or heavyweight setup.
...
```bash
mftparse --version
mftparse scan .                      # scan the current project
mftparse scan . --format json
mftparse scan . --fail-on high       # non-zero exit for CI gates
mftparse mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **MFTECmd**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`mftparse` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
