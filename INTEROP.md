...
How **mftparse** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `mftparse` lives in the **Host / artifact forensics** cluster.
...
| from | relation | to |
|---|---|---|
| `mftparse` | JSON findings compose with | [`memtriage`](https://github.com/cognis-digital/memtriage), [`evtxhunt`](https://github.com/cognis-digital/evtxhunt), [`browserforensics`](https://github.com/cognis-digital/browserforensics) |
| `mftparse` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `mftparse` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `mftparse` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the host / artifact forensics cluster.** `mftparse` output feeds the next tool:
```bash
mftparse ... --format json > out.json          # this tool's findings (see `mftparse --help`)
memtriage ... < out.json                      # the cluster sibling consumes them
```
...
mftparse ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
mftparse ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
mftparse ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `mftparse`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `mftparse` slots
