"""Native cognis-connect emit for mftparse — forward findings to any platform.

Maps mftparse's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    mftparse ... --format json | mftparse-emit --to stix
    mftparse-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        if 'file_name' in rec and 'last_written_time' in rec:
            out['title'] = f"{rec['file_name']} - Timestomp Detected"
            out['severity'] = 'medium'
            out['type'] = 'timestomp'
            out['description'] = f"File {rec['file_name']} was last written at {rec['last_written_time']}, indicating potential timestomp activity."
            out['tags'] = ['timestomp', 'suspicious']
            if 'ipv4' in rec:
                out['ipv4'] = rec['ipv4']
            if 'domain' in rec:
